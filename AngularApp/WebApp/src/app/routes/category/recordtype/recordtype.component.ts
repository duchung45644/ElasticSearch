...
import { CreateRecordTypeComponent } from './recordtype-form.component';
...
export class RecordtypeComponent implements OnInit {
    ...
    getData() {
        this.isLoading = true;
        this.recordtypeService.getByPage(this.params).subscribe((res: any) => {
    ...
    newRecordtype() {
        this.setViewId(0);
        const dialogRef = this.dialog.originalOpen(CreateRecordTypeComponent, {
            width: '800px',
            data: { viewId: 0 },
        });
    ...
    deletePosition(value: any) {
        this.recordtypeService.delete({ Id: value.Id }).subscribe((data: any) => {
            this.getData();
