...
import { CreateCatalogComponent } from './catalog-form.component';
...
export class CatalogComponent implements OnInit {
    columns: MtxGridColumn[] = [
        ...
        { header: 'Danh mục hồ sơ', field: 'Name', width: '300px', sortable: true },
            ...
            buttons: [
                {
                    ...
                    tooltip: 'Cập Nhật',
                    type: 'icon',
                    click: (record) => this.editCatalog(record),
                },
                {
                    icon: 'delete',
                    tooltip: 'Xoá',
                    ...
                    popCloseText: 'Đóng',
                    popOkText: 'Đồng ý',
                    click: (record) => this.deleteCatalog(record),
                },
            ],
        },
    ];
    ...
    getData() {
        this.isLoading = true;
        this.catalogService.getByPage(this.params).subscribe((res: any) => {
    ...
    newCatalog() {
        this.setViewId(0);
        const dialogRef = this.dialog.originalOpen(CreateCatalogComponent, {
            width: '800px',
            data: { viewId: 0 },
        });
    ...
    deleteCatalog(value: any) {
        this.catalogService.deleteCatalog({ Id: value.Id }).subscribe((data: any) => {
            this.getData();
            this.toastr.success(`Đã xoá ${value.Name}!`);
        });
    }

    editCatalog(value: any) {
