...
import { ViewDetailsHistory } from './view-details';

@Component({
    selector: 'app-document-return-history',
    templateUrl: './document-return-history.component.html',
    styleUrls: ['./document-return-history.component.scss'],
})
export class DocumentReturnHistoryComponent implements OnInit {
    isLoading = false;
    viewId: number;
    ...
    HistoryListColumns: MtxGridColumn[] = [
        ...
        {
            header: 'Trạng thái',
            field: 'ReturnStatusHistory',
            type: 'tag',
            width: '120px',
            tag: {
                1: { text: 'Trả hết', color: 'blue-100' },
                2: { text: 'Trả một phần', color: 'orange-100' },
            },
        },
        {
            header: 'Chức Năng',
    ...
    getData() {
        this.isLoading = true;
        this.approvalManagementService.getDocumentReturnHistory(this.params).subscribe((res: any) => {
            this.total = res.Data.Pagination.NumberOfRows;
            this.borrowSlipList = res.Data.ListObj;

            this.isLoading = false;
            this.cdr.detectChanges();
            console.log(this.borrowSlipList.length);
        });
    }

    ViewDetails(value: any) {
        const dialogRef = this.dialog.originalOpen(ViewDetailsHistory, {
            width: '1200px',
            data: { viewId: value.Id, historyId: value.HistoryId },
        });
