...
@Component({
    ...
    templateUrl: './approveedit.html',
})
export class ApproveEditComponent {
    ...
    constructor(
        ...
        public dialogRef: MatDialogRef<ApproveEditComponent>,
    ...
    getallrecord() {
        ...
        this.approveService
            ...
            .subscribe((data: any) => {
                this.recordlistssss = data.Data.Approves;
    ...
    DestroyRecords() {
        // if (this.record.Selected = false) {
        if (this.recordlistssss == undefined || this.recordlistssss.length < 0) {
            this.toastr.error(`Vui lòng chọn hồ sơ!`);
            return;
        }

        var listRecordCheck = [];
        for (var i = 0; i < this.recordlistssss.length; i++) {
            var recordlistss = this.recordlistssss[i];
            console.log(recordlistss.Selected);
            if (recordlistss.Selected) {
                listRecordCheck.push(recordlistss.RecordId);
            }
        }

        this.approveService.DeleteRecordInApprove({ ListRecord: listRecordCheck }).subscribe((data: any) => {
            this.toastr.success(`Đã xóa hồ sơ khỏi đợt thành công!`);
            this.getData();
            this.getdatabypage();
        });
    }
